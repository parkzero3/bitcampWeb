...
<style>
	...
	#paging li{
		float:left; width:60px; height:40px; text-align:center; font-size:1em;
	}
	...
	<div id="paging">
		<ul>
			...
			<% if(nowPage==1){ %>
				Prev
			<%}else{ %>
			<a href="/bitcamp/freeboard/boardList.jsp?nowPage=<%=nowPage-1%>">Prev</a>
			<%} %>
			</li>
			<!-- 페이지 넘버 매기기 -->
			...
			%>
			<li>
				<a href="/bitcamp/freeboard/boardList.jsp?nowPage=<%=p%>" style="color:red"><%=p %></a>
			</li>
			<% 		}else{		%>
				<li>
				<a href="/bitcamp/freeboard/boardList.jsp?nowPage=<%=p%>"><%=p %></a>
				</li>
			<% 		
					}
				} //if 
			...
			<!-- 다음 페이지 -->
				...
				%>
					<a href="/bitcamp/freeboard/boardList.jsp?nowPage=<%=nowPage+1%>">Next</a>
				<% } %>
