	...
	public List<FreeboardVO> getAllRecord(int nowPage, int onePageRecord,int totalPage, int totalRecord) {
		...
		try {
			...
			String sql = "select * from"
					+ " (select * from"
					+ " (select freeboard.no, freeboard.subject, freeboard.userid, freeboard.hit,"
					+ " to_char(freeboard.writedate,'YYYY-MM-DD') writedate, register.username  "
					+ " from freeboard join register on freeboard.userid = register.userid order by freeboard.no desc)"
